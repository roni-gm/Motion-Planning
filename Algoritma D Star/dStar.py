...
class DStar(Dijkstra):
    """
    Simple D* Lite implementation (static run) to compute shortest path
    from start to goal on the current grid. This is a functional D*-like
    planner that uses rhs/g values and a priority queue; it can be
    extended later for dynamic replanning.
    """
    def planning(self, sx, sy, gx, gy):
        # convert to grid indices
        start = (self.calc_xy_index(sx, self.min_x), self.calc_xy_index(sy, self.min_y))
        goal = (self.calc_xy_index(gx, self.min_x), self.calc_xy_index(gy, self.min_y))
        ...
        def neighbors(u):
            ...
            for dx, dy, cost in self.motion:
                vx, vy = ux + dx, uy + dy
                nnode = self.Node(vx, vy, 0, -1)
                if self.verify_node(nnode):
                    res.append(((vx, vy), cost))
            return res

        def h(u, v=start):
            # heuristic: Euclidean on grid indices
            return math.hypot(u[0] - v[0], u[1] - v[1])

        def key(u):
            gu = g.get(u, INF)
            ru = rhs.get(u, INF)
            k1 = min(gu, ru) + h(start, u)
            k2 = min(gu, ru)
            return (k1, k2)
        ...
        def pq_top_key():
            while pq:
                k1, k2, _, u = pq[0]
                if entry_finder.get(u, None) == pq[0]:
                    return (k1, k2)
                heapq.heappop(pq)
            return (INF, INF)
        ...
        def update_vertex(u):
            if u != goal:
                ...
                for (v, cost) in neighbors(u):
                    val = cost + g.get(v, INF)
                    if val < min_rhs:
                        min_rhs = val
                rhs[u] = min_rhs
            if entry_finder.get(u) is not None:
                pq_remove(u)
        ...
        def compute_shortest_path():
            while True:
                topk = pq_top_key()
                k_start = key(start)
                if topk >= k_start and rhs.get(start, INF) == g.get(start, INF):
                    break
                # pop best
                while pq:
                    k1, k2, _, u = heapq.heappop(pq)
                    if entry_finder.get(u, None) == (k1, k2, _, u):
                        entry_finder.pop(u, None)
                        break
                else:
                    break
        ...
        path_y = []
        current = start
        if g.get(current, INF) == INF and rhs.get(current, INF) == INF:
            return [], []
        path_x.append(self.calc_position(current[0], self.min_x))
        path_y.append(self.calc_position(current[1], self.min_y))
        max_iter = (self.x_width + self.y_width) * 10
        it = 0
        while current != goal and it < max_iter:
            it += 1
            # choose neighbor minimizing cost + g
            best = None
            best_val = INF
