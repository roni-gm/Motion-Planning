...
def create_gen(target_length): 
    random_number = np.random.randint(32, 126,  
        size=target_length) 
...
def create_population(target, max_population, target_length): 
    ...
    for i in range(max_population): 
        ...
        genfitness = calculate_fitness(gen, target,  
                     target_length) 
...
def display(parent): 
    ...
    print('{}\t{}%\t{}'.format(bestgen(parent),  
          round(bestfitness(parent), 2), timeDiff)) 
...
print('----------------------------------------------') 
population = create_population(target, int(max_population),  
             target_length) 
...
while 1: 
    ...
    mutant = mutation(child, target, float(mutation_rate),  
             target_length) 
